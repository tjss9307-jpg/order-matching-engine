...
void feedGenerator::runLiveFeed(OrderQueue& ordQ, int num_orders){
    ...
    std::normal_distribution<double> price_dist(100.00 , 0.5); // mean = 100 , sd = 0.5

    
    
    for (int i=1; i<=num_orders; i++){
        Side rndm_side = (side_dist(engine)==0) ? Side::BUY : Side::SELL;
        int rndm_quantity = quantity_dist(engine);
        double rndm_price = price_dist(engine);
